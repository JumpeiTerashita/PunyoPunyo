...
SceneBattle::SceneBattle()
{
	...
	_is_vertical = true;
	GameOver = false;
	setSequence(&SceneBattle::Playing);
}
...
SceneBattle* SceneBattle::getInstance() {
	if (nullptr == SceneBattle::instance) {
		SceneBattle::instance = new SceneBattle();
	}
	return SceneBattle::instance;
}
...
void SceneBattle::PuyoCreate()
{
	First = new BattlePuyo(3, 12, 4);
	//First->Status |= Is_rotate;
	Second = new BattlePuyo(3, 13, 4);
	Second->_is_rotate = true;
}

void SceneBattle::CreatedMap()
{
	if (map[12][3] != nullptr)
	{
		GameOver = true;
...
void SceneBattle::CheckPuyo(int _x, int _y)
{
...
void SceneBattle::DeleteStart()
{
	...
	VanishPuyo();
	//if (VanishCounter == 0) 
	if (!DeletePuyoStatus) setSequence(&SceneBattle::FinishedVanish);
	else
	{
		DelScoreCalc();
		ChainCounter++;
...
void SceneBattle::DeleteMarkSet()
{
	if (PuyoCounter >= 4)
	{
		for (int i = 1; i < 15; i++)
	...
	for (int i = 1; i < 15; i++)
	{
		for (int j = 1; j < 7; j++)
		{
			if (map[i][j] != nullptr)
			{
				...
				{
					COLORPATTERN_BATTLE DelColor = map[i][j]->ColorNumber;
				...
				{
					COLORPATTERN_BATTLE RefallColor = map[i][j]->ColorNumber;
					map[i][j] = nullptr;
					BattlePuyo* Refall = new BattlePuyo(j, i, RefallColor);
					Refall->_is_falling = true;
					Refall->_is_freefall = true;
					GameManager::getInstance()->addObject(Refall);
					Refall->setSequence(&BattlePuyo::FreeFall);
					map[i][j] = Refall;
				}
			}
		}
...
void SceneBattle::WaitingRestart()
{
...
void SceneBattle::Playing()
{
	...
	{
		WaitingRestart();
		setSequence(&SceneBattle::WaitingRestart);
	}
	if (GameManager::getInstance()->getObject().size() == 0)
	{
		setSequence(&SceneBattle::DeleteStart);
	}
}

void SceneBattle::FinishedVanish()
{
	ChainCounter = 0;
	CreatedMap();
	PuyoCreate();
...
void SceneBattle::update()
{
	if (GameManager::getInstance()->getObject().size() == 1)
	{
		...
		BattlePuyo* puyoit = dynamic_cast<BattlePuyo*>(*GameManager::getInstance()->getObjectsIterator());
		puyoit->_is_freefall = true;
	}
	runSequence();
}

void SceneBattle::DelScoreCalc()
{
	//TODO 同時消しボーナス追加
	...
	for (int i = 0; i < 4; i++)
	{
		ColorResultStatus[i] = BitColPicker((COLORPATTERN_BATTLE)i, VanishedPuyo);
